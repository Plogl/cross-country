...
public class CS400GraphMINE<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    public CS400GraphMINE() { vertices = new Hashtable<>(); }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	...
        	for(NodeType data: copyPath.dataSequence) {
        		//makes sure data is copied twice.
        		if(!(dataSequence.contains(data))) {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    	//checks vertices exist
    	if (!this.containsVertex(start) || !this.containsVertex(end))
    		throw new NoSuchElementException("Ther are no found verticies containing start and/or end values.");
    	//created priority queue for discovered paths
    	PriorityQueue<Path> discovered = new PriorityQueue<Path>();
    	// first path (start -> start)
    	Path startPath = new Path(new Vertex(start)); 
    	//sets current and target vertices.
    	Vertex currentVertex = this.vertices.get(start);
    	Vertex targetVertex = this.vertices.get(end);
    	//adds Edges of current Vertex
    	for (Edge e: currentVertex.edgesLeaving) {
    		discovered.add(new Path(startPath, e));
    	}
    	//creates visited Hashtable for vertexs
    	Hashtable<Vertex, Double> visited = new Hashtable<Vertex, Double>();
    	visited.put(currentVertex, startPath.distance);
    	//creates 
    	Path currentPath = startPath;
    	while(!discovered.isEmpty() && currentVertex != targetVertex) {
    		//gets path
    		currentPath = discovered.peek();
    		//finds target of currentPath
    		currentVertex = discovered.poll().end;
    		//checks to see if target vertex is already in visited
    		if(visited.containsKey(currentVertex)) {
    			continue;
    		}
    		//checks if currentVertex is targetVertex
    		if(currentVertex != targetVertex) {
    			//adds all of currentVertex edges to discovered queue
    			for (Edge e: currentVertex.edgesLeaving) {
    	    		discovered.add(new Path(currentPath, e));
    	    	}
    		}
    		//adds currentVertex to visited 
    		visited.put(currentVertex, currentPath.distance);
    	}
    	//checks to targetVertex was ever found.
    	if (!visited.containsKey(targetVertex))
    		throw new NoSuchElementException("Cannot find any path between given start and end vertices"); 		
        return currentPath;
