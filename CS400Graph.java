...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            {
                if (dataSequence.contains(data) == false)
                {
    ...
    //Names things as descriptively as I could so as to avoid needing to write a lot of comments, as I find that
    //in terms of readability this is cleaner
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        if (this.containsVertex(start) == false || this.containsVertex(end) == false)
        {
        ...
        LinkedList<Path> currentPaths = new LinkedList<>();
        LinkedList<Vertex> visitedVertices = new LinkedList<>();
        LinkedList<Path> failedPaths = new LinkedList<>();

        currentPaths.add(new Path(vertices.get(start)));
        while (currentPaths.size() != 0)
        {
            for (int i = 0; i < currentPaths.size(); i++)
            {
                ...
                if(path.end == vertices.get(end))
                {
                    completedPaths.add(path);
                    currentPaths.remove(path);
                    break;
                }
                else if(visitedVertices.contains(path.end) == false && path.end.edgesLeaving.size() > 0
                        && path.end.equals(vertices.get(end)) == false)
                {
                    LinkedList<Edge> tempEdge = path.end.edgesLeaving;
                    visitedVertices.add(path.end);

                    for(Edge edge: tempEdge)
                    {
                        currentPaths.add(new Path(path, edge));
                    }
                    currentPaths.remove(path);
                    break;
                }
                else if(path.end.equals(vertices.get(end)) == false)
                {
                    currentPaths.remove(path);
